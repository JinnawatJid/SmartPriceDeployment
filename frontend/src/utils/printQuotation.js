...
const c = (v) => (Number.isFinite(v) ? v.toLocaleString('th-TH', { style: 'currency', currency: 'THB' }) : '');
...
const POS = {
  ...
  cols: {
    codeL:   60,  codeR:   120,   // Code (ชิดซ้าย)
    nameL:   130, nameR:   350,   // Name (ตัดคำ)
    qtyR:    380,               // Qty (ชิดขวา)
...
export async function printQuotation({ state, calculation, employee }) {
  ...
  page0.drawText(String(customer.name || ''), { x: POS.custName.x,     y: POS.custName.y,     size: 11, font });
  page0.drawText(String(customer.phone || ''),{ x: POS.custPhone.x,    y: POS.custPhone.y,    size: 11, font });
  ...
  page.drawText(`รวมจำนวนเงิน (สินค้า) : ${st}`, { x: POS.subTotal.x,   y: POS.subTotal.y,   size: sizes.totals, font: fontBold });
  page.drawText(`ค่าจัดส่ง : ${ship}`,             { x: POS.shipping.x,   y: POS.shipping.y,   size: sizes.totals, font });
  page.drawText(`ภาษีมูลค่าเพิ่ม : ${vat}`,       { x: POS.vat.x,        y: POS.vat.y,        size: sizes.totals, font });
  page.drawText(`จำนวนเงินรวมสุทธิ : ${grand}`,    { x: POS.grandTotal.x, y: POS.grandTotal.y, size: sizes.totalsBold, font: fontBold });

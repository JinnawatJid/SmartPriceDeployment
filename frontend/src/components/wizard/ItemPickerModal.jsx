...
function ItemPickerModal({ open, category, onClose, onConfirm }) {
  ...
  const handleAdd = (item, qty) => {
    ...
    const normalizedItem = {
      ...
      pkg_size: item.pkg_size ?? item.pkgSize ?? item["pkg_size"] ?? 1,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 overflow-y-auto">
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-xl p-8 animate-fadeIn">
        ...
        <div className="flex justify-between items-center pb-3 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-800">เลือกสินค้า: {category}</h2>
          <button onClick={onClose} className="px-3 py-1 text-sm text-gray-600 hover:text-gray-900">
            ✕
          </button>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4">
          <div className="space-y-3">
            ...
            {category === "A" && (
              <AluminiumPicker onSelect={(filters) => setAluFilter(filters || {})} />
            )}
            {category === "C" && (
